...
impl<'a, 'b, const N: usize> MetricEncoder<'a, 'b, N> {
    fn new(writer: &'a mut String, ns: &str, info: &'b Info<N>) -> Self {
        let name = format!(
            "{}_{}_{}{}{}",
            ns,
            info.subsys,
...
impl Encoder {
    ...
    pub fn write<T: fmt::Display>(&mut self, info: &Info<0>, val: T) {
        let mut menc = MetricEncoder::new(&mut self.writer, self.ns, info);
        menc.write(&[], val);
    }
    ...
    pub fn with_info<'a, 'b, const N: usize>(
        ...
        MetricEncoder::new(&mut self.writer, self.ns, info)
    }
