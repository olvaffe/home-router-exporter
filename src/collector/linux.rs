...
impl Linux {
    ...
    pub fn collect(&self, prom: &Prom, enc: &mut metric::Encoder) {
        if let Err(err) = self.collect_cpu(&prom.metrics, enc) {
            error!("failed to collect cpu metrics: {err:?}");
        }

        if let Err(err) = self.collect_mem(&prom.metrics, enc) {
            error!("failed to collect mem metrics: {err:?}");
        }

        if let Err(err) = self.collect_fs(&prom.metrics, enc) {
            error!("failed to collect fs metrics: {err:?}");
        }

        if let Err(err) = self.collect_thermal(&prom.metrics, enc) {
            error!("failed to collect thermal metrics: {err:?}");
        }

        if let Err(err) = self.collect_net_link_speed(&prom.metrics, enc) {
            error!("failed to collect net link speed: {err:?}");
        }

        if let Err(err) = self.collect_net_link_state(&prom.metrics, enc) {
            error!("failed to collect net link state: {err:?}");
        }

        if let Err(err) = self.collect_net_route(&prom.metrics, enc) {
            error!("failed to collect net route: {err:?}");
        }

        if let Err(err) = self.collect_net_nft(&prom.metrics, enc) {
